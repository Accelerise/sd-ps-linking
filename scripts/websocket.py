        ...
        with gr.Row(elem_id = elem + 'row'):
            with gr.Accordion('WebSocket', open=False, elem_id=elem+'accordion'):
                with gr.Row(elem_id=elem+'route_row'):
                    gr_route = gr.Textbox(label='地址', value='ws://localhost:7860/ws/client/sd', elem_id=elem+'route', interactive=True)
                    gr_enable = gr.Checkbox(label='启用', value=lambda: is_img2img, elem_id=elem+'enable', interactive=True)
