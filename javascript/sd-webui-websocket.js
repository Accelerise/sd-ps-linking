...
// receive and send
function sync() {
    console.log('sync', new Date().toString())
    const tab = getTargetTab();
    // check enable status
    ...
    // send
        ...
        console.log('sync addToSend', new Date().toString())
        addToSend();
    }
    // refresh
    else {
...
function openWebsocket(route) {
    ...
    ws.onmessage = async function (e) {
        ...
        if (msg.type === 'check_health') {
            console.log('toSend push checkHealth', new Date().toString())
            toSend.push(JSON.stringify(checkHealth(msg)))
        }
        if (msg.type === 'log') {
            // pass
        }
    ...
    ws.onerror = function (e) {
        ...
        toReceive.push(JSON.stringify({ type: 'error', code: e.code, message: e.message, e }));
    }
    return ws;
}
